...
function ExportConnector() {
  const { currentSplit, clearCurrentSplit, deleteSplit, markSplitForUpgrade } = useSplits();
  ...
  return (
    <ExportScreen
      ...
      onSaveToAccount={async () => {
        await markSplitForUpgrade(currentSplit.id);
        showToast({ message: 'Sign in to save this split', variant: 'info', duration: 3000 });
...
export default function GuestHomeStack() {
  return (
    <Stack.Navigator
      screenOptions={{
        headerShown: false,
